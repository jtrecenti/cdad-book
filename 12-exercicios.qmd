...
print("SETUP ok — colunas:", list(camaras.columns)[:10])
```

```{pyodide}
#| exercise: ex_1
# Escreva sua solução aqui:
# Agrupe as decisões usando value_counts() e renomeie a coluna de contagem.
```

::: { .hint exercise="ex_1" }
::: { .callout-note collapse="false" }
## Dica 1
Use `camaras["decisao"].value_counts()` para contar quantas vezes cada tipo de decisão aparece.
:::
:::
    ...
    if isinstance(result, pd.DataFrame):
        ...
        if "decisao" in res.columns or "index" in res.columns:
            ...
            if ok_cols and len(res) == len(expected):
                ...
                feedback = {"correct": False, "message": "As colunas ou contagens não correspondem. Verifique o uso de value_counts() e reset_index()."}
        else:
            feedback = {"correct": False, "message": "O DataFrame deve conter a coluna 'decisao' e a contagem ('qtd_decisoes')."}
    else:
        feedback = {"correct": False, "message": "Retorne um DataFrame (total_decisoes) para validação."}
...
try:
    ...
    if isinstance(result, pd.DataFrame):
        ...
        feedback = {"correct": False, "message": "Retorne o DataFrame total_decisoes_por_camara antes do gráfico para validação."}
except Exception as e:
    feedback = {"correct": False, "message": f"Erro ao validar: {e}"}
feedback
```
...
## Solução
```python
rel_comparacao = ['MÁRIO DEVIENNE FERRAZ', 'ALBERTO ANDERSON FILHO']
...
#| exercise: ex_3
#| check: true
# Checador simples — verifica se o DataFrame 'rel_comparacao' foi criado corretamente.
import pandas as pd
feedback = None
try:
    ...
    if isinstance(result, pd.DataFrame):
        if 'RELATOR' in result.columns:
            ...
            if relatores_encontrados == relatores_esperados:
                ...
                feedback = {"correct": False, "message": f"Os relatores filtrados não correspondem. Esperado: {relatores_esperados}"}
        else:
            feedback = {"correct": False, "message": "Seu DataFrame precisa conter a coluna 'RELATOR'."}
    else:
        feedback = {"correct": False, "message": "Retorne o DataFrame rel_comparacao antes do gráfico para validação."}
except Exception as e:
    feedback = {"correct": False, "message": f"Erro ao validar: {e}"}
feedback
```
...
#| exercise: ex_4
#| check: true
# Checador automático — verifica se 'decisoes_idade' contém colunas esperadas e merge correto.
import pandas as pd
feedback = None
try:
    # monta versão esperada mínima
    ...
    if isinstance(result, pd.DataFrame):
        ...
        if cols_necessarias.issubset(set(result.columns)):
            ...
            feedback = {"correct": False, "message": f"Faltam colunas esperadas: {faltando}. Verifique a junção e nomes das colunas."}
    else:
        feedback = {"correct": False, "message": "Retorne o DataFrame (decisoes_idade) antes do gráfico para validação."}
except Exception as e:
    feedback = {"correct": False, "message": f"Erro ao validar: {e}"}
feedback
```
...
A **análise empírica da atuação judicial** permite discutir até que ponto **fatores pessoais**, como **idade** e **tempo de carreira**, influenciam os resultados das decisões.  

Embora **correlação não implique causalidade**, tais estudos são fundamentais para compreender a **diversidade de perfis decisórios** e para fomentar **políticas de uniformização jurisprudencial** e **gestão da magistratura**.
:::
...
#| exercise: ex_5
#| check: true
# Checador automático — valida se o DataFrame 'relatores' contém as colunas esperadas.
import pandas as pd
feedback = None
try:
    ...
    if isinstance(result, pd.DataFrame):
        ...
        feedback = {"correct": False, "message": "Retorne o DataFrame (relatores) antes do gráfico para validação."}
except Exception as e:
    feedback = {"correct": False, "message": f"Erro ao validar: {e}"}
feedback
```
...
O **cruzamento entre idade e tempo de magistratura** é útil não apenas para a **validação dos dados**, mas também para refletir sobre **trajetórias profissionais** e a formação de **cultura institucional** dentro do **Poder Judiciário**.  

Ao compreender o **perfil de carreira dos relatores**, a equipe pode investigar com mais profundidade como a **experiência influencia padrões decisórios** e a **estabilidade jurisprudencial**.
:::
